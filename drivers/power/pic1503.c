...
** liaofuchun@EXP.Driver 	  2013/02/11    1.10	modify the value of(vbus - vbatt)
** liaofuchun@EXP.Driver	  2013/02/14	1.11	when temp over,send NOTIFY_TEMP_OVER to AP instead of NOTIFY_FAST_ABSENT
** ------------------------------------------------------------------------------
 */
...
char tx_data_to_ap(char info)
{
	...
	for( i = 0; i < 7; i++)
	{	
		//lfc add to avoid both RC5_1503 and RC5_1823 are in output direction
...
{
	static char tm0_count = 0, tm1_count = 0,ra5_first_int = 0;
	if(SSP1IF){
		SSP1IF = 0;
		i2c_int_finished = 0;
		return ;
...
//step1: PORT config
	...
	TRISA0 = 1;
	ANSA0 = 1;
	//lfc delete for use RA1 as interrupt
	TRISA1 = 1;
	ANSA1 = 1;
	//lfc delete for use RA1 as interrupt end
	TRISA2 = 1;	//set RA2 input
	ANSA2 = 1;	//set RA2 analog input
//step2: ADC module config
	ADCON1 = 0xF0;	//right justify,frc osc
	if(adc_chan == 0){	//VBAT on battery
		ADCON0 = 0x01;	//select ADC_CHAN AN0(RA0),turn on ADC
	}
	else if(adc_chan == 1){	//VBAT on PCB board
		ADCON0 = 0x05;	//select ADC_CHAN AN1(RA1),turn on ADC
	}	
	else if(adc_chan == 2){	//VBUS VOL
		ADCON0 = 0x09;	//select ADC_CHAN AN2(RA2),turn on ADC
	}
	else if(adc_chan == 3){	//RC2 GND VOL
...
char vbus_vbatt_detect(char adc_chan1,char adc_chan2)
{
	...
	vol1 = vol1 - vol2;
	rc = vol_compare_result(adc_chan1,vol1);
	return rc;
}
*/
/*
	...
	switch(adapter_data){
		...
		case IS_VOLT_OK:	//adjust current--level 2
			if(!mcu_i){
				...
				vbus_detect = vbus_vbatt_detect(ADC_RA2,ADC_RA0);	//330 words	//RA2 - RA1
...
void port_init(void)
{
	//RA4 output clk,setted in config bit
	//if it is setted in code,must be disabled in config bit
	//pull up RC3(close fastchg)
	//CLKOUTEN_ON = 0;	//enable clkouten on RA4
	RC3 = 0;
	LATC3 = 0;
	TRISC3 = 0;	//set RC3 output
	ANSC3 = 0;	//set RC3 digital I/O
	RC3 = 1;
/*
	//config RA3(clk from ap) posedge interrupt trigger
	//forbid RA3 int init
	IOCAP3 = 0;	//posedge trigger
	IOCAN3 = 0;	//forbid negedge trigger
	IOCIE = 1;	//enable level interrupt
	GIE = 1;	//enable global interrupt
	IOCIF = 0;	//clear level interrupt flag
*/	
	...
	/*
	//lfc open RA1 int at first for ap ask 1503 firmware version
	TRISA1 = 1;
	ANSA1 = 0;
	IOCAF1 = 0;
	...
	IOCIE = 1;
	GIE = 1;
	//nWPUEN = 0;	//enable all pins wake_pullup
	//WPUA2 = 1;
	*/
	//config RC5 input
	CLC1CON = 0;
	CLC1SEL = 0;	//disable clc1 on rc5
	CWG1CON0 = 0;
	CWG1CON1 = 0;
	CWG1CON2 = 0;
	RC5 = 0;	//it is must,otherwise RC5 will corrupt randomly in release version
	LATC5 = 0;	//it is must,otherwise RC5 will corrupt randomly in release version	
	TRISC5 = 1;	//set RC5 input
	
	//config RA5 posedge interrupt trigger
	//TRISA5 = 1;	//set RA5 input
	//ANSA5 = 0;	//set RA5 digital I/O
	LATA5 = 0;
	TRISA5 = 1;
	RA5 = 0;
	IOCAP5 = 1;	//posedge trigger
	IOCAN5 = 0;	//forbid negedge trigger
	IOCIE = 1;	//enable level interrupt
	GIE = 1;	//enable global interrupt
	IOCIF = 0;	//clear level interrupt flag

	//forbid RA3 interrupt in init
	//IOCAP3 = 1;		//forbid posedge trigger
	//IOCAN3 = 0;		//negedge trigger

	//config RC4 output low_level digital I/O
	//TRISC4 = 0;
	//RC4 = 0;
	//LATC4 = 0;
	
	//lfc delete just for debug begin
	//config RA1 input digital I/O
/*
	TRISA1 = 1;
	ANSA1 = 0;	//digital I/O
	IOCAP1 = 1;	//enable RA1 int init
...
if(i > 55){
	...
	if((vbat_temp_cur < 100) || (vbat_temp_cur > 450)){ //change temp range to 10-45degree
	...
	if(j > 150){
		vbat_temp_cur = bq27541_get(BQ27541_REG_CURRENT);	//it need to cost 1600ms to read current correctly
...
//const int fw_exist @ 0x7ff = 0x3455;
void main(void)
{
	char rc = 0;
	pic1503_init();
...
*/
	...
	//	while(!allow_chg);
	//enter fastchg mode
	while(RC3){	//wait until RC3 pulled down(start fastchg)
		asm("clrwdt");
		if(need_to_reset){
			#asm;
			RESET;
