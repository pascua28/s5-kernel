			...
			size_t count)
{
	...
	scnprintf(store_address, SMEM_IMAGE_VERSION_NAME_SIZE - 1, "%-.75s", buf);
	return count;
}
			...
			size_t count)
{
	...
	scnprintf(store_address, SMEM_IMAGE_VERSION_VARIANT_SIZE - 1, "%-.20s", buf);
	return count;
}
			...
			size_t count)
{
	...
	scnprintf(store_address, SMEM_IMAGE_VERSION_OEM_SIZE - 1, "%-.32s", buf);
	return count;
}
